...
class XinpcSpider(scrapy.Spider):
    name = 'xinpc'
    ...
    def parse_post(self, response):
        ...
        categorys = response.xpath('//div/span[contains(@class, "cate")]//text()').extract()
        ...
        post['create_time'] = response.xpath('//div/span[contains(@class, "update-time")]/i/text()').get()
        post['play_counts'] = str_int(response.xpath('//div/i[contains(@class,"play-counts")]/text()').get())
        post['like_counts'] = str_int(response.xpath('//div//span[contains(@class,"like-counts")]/@data-counts').get())
        post['description'] = strip(response.xpath('//div//p[contains(@class,"desc")]/text()').get())
        request = Request(video_url % vid, callback=self.parse_video)
        # request = response.follow(video_url % vid, self.parse_video)
    ...
    def parse_comment(self, response):
        ...
        for c in result['data']['list']:
            ...
            comment['username'] = c["userInfo"]["username"]
