...
from apis.api_constant import HTTP_CREATE_OK
from apis.movie_user.utils import movie_users_login, movie_users_permission
from models.cinema_admin.cinema_hall_model import Hall
from models.cinema_admin.cinema_hall_movie_model import HallMovie
from models.movie_user.movie_order_model import MovieOrder, ORDER_STATUS_PAYED_NOT_GET, ORDER_STATUS_GET, \
    ORDER_STATUS_NOT_PAY
from models.movie_user.movie_user_model import VIP_USER
...
class MovieOrdersResource(Resource):
    ...
    def post(self):
        ...
        for movie_orde in movie_orders_buyed:
            sold_seats = movie_orde.o_seats.split('#')
            seats.extend(sold_seats)

        for movie_orde in movie_orders_lock:
            sold_seats = movie_orde.o_seats.split('#')
            seats.extend(sold_seats)
